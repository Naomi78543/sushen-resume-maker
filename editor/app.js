(() => {
  ...
  const TEMPLATE_URL = "../skills/sushen-resume-maker/assets/resume_template.html?v=20260907-highlight-restore";
  ...
  function areaBulletEditor(title, items, options = {}) {
    ...
    // 手动重点词:输入后匹配到各行文本中的词,写入该行 highlights(与自动识别结果合并)
    const highlightRow = element("div", { className: "bullet-highlight-row" });
    const highlightInput = element("input", { className: "bullet-highlight-input" });
    highlightInput.type = "text";
    highlightInput.placeholder = "重点词（逗号分隔，仅对文本中确实出现的词生效）";
    const joinedHighlights = () => {
      const all = [];
      (items || []).forEach(item => {
        if (item && typeof item === "object") (item.highlights || []).forEach(w => { if (!all.includes(w)) all.push(w); });
      });
      return all.join(", ");
    };
    highlightInput.value = joinedHighlights();
    highlightInput.addEventListener("input", () => {
      const words = highlightInput.value.split(/[,，]/).map(x => x.trim()).filter(Boolean);
      (items || []).forEach((item, i) => {
        const text = typeof item === "string" ? item : (item && item.text) || "";
        if (typeof items[i] === "string") items[i] = bulletObject(text);
        if (!items[i] || typeof items[i] !== "object") return;
        const matched = words.filter(w => text.includes(w));
        items[i].highlights = [...new Set([...matched, ...detectHighlights(text)])];
      });
      scalarChanged();
    });
    const highlightBtn = element("button", { className: "mini-button", text: "应用重点词", type: "button" });
    highlightBtn.addEventListener("click", () => {
      highlightInput.dispatchEvent(new Event("input"));
      showToast("重点词已应用到各条目");
    });
    highlightRow.append(highlightInput, highlightBtn);
    wrap.append(highlightRow);

    // 说明行(轻量,不占用太多空间)
    wrap.append(element("p", { className: "field-hint", text: "每行将作为独立一条渲染；工具栏操作作用于光标所在行或多行选区；重点词仅匹配各行文本中出现的词。" }));
    return wrap;
  }
